...
-(void)readFromCSV:(NSString*)csvPath  TitleAtColumn:(int)tcolumn  DataAtColumn:(int)dColumn
{
   ...
   // linesArray=nil;
    int a1=0,a2=0,a3=0,a4=0,a5=0,a0=0;
    if(!groupBars){
    NSMutableArray *arrayRet,*arrayRetSub;
    arrayRet=[dal SelectWithStar:@"tblMainTask"];
    arrayRetSub=[dal SelectWithStar:@"tblSubTask"];
    NSMutableArray *arrDates=[[NSMutableArray alloc] init];
    for(int i=0;i<[arrayRet count];i++){
        [arrDates addObject:[[arrayRet objectAtIndex:i] valueForKey:@"alertTime"]];
    }
    for(int i=0;i<[arrayRetSub count];i++){
            [arrDates addObject:[[arrayRetSub objectAtIndex:i] valueForKey:@"alertTime"]];
    }
    NSLog(@"%@",arrDates);
    
    NSDate *today=[NSDate date];
    for(int a=0;a<[arrDates count];a++){
        if([[arrDates objectAtIndex:a] isEqualToString:@""]){
            
        }
        else{
            NSString *strFin=[arrDates objectAtIndex:a];
            strFin = [strFin substringToIndex:[strFin length] - 14];
    ...
    [_yElements addObject:[NSString stringWithFormat:@"%d",a0]];
    [_yElements addObject:[NSString stringWithFormat:@"%d",a1]];
    [_yElements addObject:[NSString stringWithFormat:@"%d",a2]];
    [_yElements addObject:[NSString stringWithFormat:@"%d",a3]];
    [_yElements addObject:[NSString stringWithFormat:@"%d",a4]];
    [_yElements addObject:[NSString stringWithFormat:@"%d",a5]];
    ...
    [_xElements addObject:@"Today"];
    [_xElements addObject:@"After 1 Day"];
    [_xElements addObject:@"After 2 Days"];
    [_xElements addObject:@"Today"];
    [_xElements addObject:@"After 1 Day"];
    [_xElements addObject:@"After 2 Days"];
    NSLog(@"%@",_xElements);
...
-(void)getXElemets{
    ...
    arrayRetSub=[dal SelectWithStar:@"tblSubTask"];
    NSMutableArray *arrDates=[[NSMutableArray alloc] init];
    NSMutableArray *arrStat=[[NSMutableArray alloc] init];
    for(int i=0;i<[arrayRet count];i++){
        [arrDates addObject:[[arrayRet objectAtIndex:i] valueForKey:@"alertTime"]];
        [arrStat addObject:[[arrayRet objectAtIndex:i] valueForKey:@"status"]];
    }
    for(int i=0;i<[arrayRetSub count];i++){
        [arrDates addObject:[[arrayRetSub objectAtIndex:i] valueForKey:@"alertTime"]];
        [arrStat addObject:[[arrayRet objectAtIndex:i] valueForKey:@"status"]];
    ...
    for(int a=0;a<[arrDates count];a++){
        if([[arrDates objectAtIndex:a] isEqualToString:@""]){
            ...
            NSDateComponents *difference = [calendar components:NSWeekCalendarUnit|NSWeekdayCalendarUnit
                                                       fromDate:fromDate toDate:toDate options:0];
            
            
            NSLog(@"%d",[difference week]);
            if([difference day]==0){
                //a0++;
            }
            else if([difference day]==1){
